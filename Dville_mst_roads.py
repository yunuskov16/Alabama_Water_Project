...
for index, row in nodes.iterrows():
    ...
    counter_N+=1
G = nx.relabel_nodes(G, N_list_df)

edges = ox.graph_to_gdfs(G, nodes=False, edges=True)
nodes = ox.graph_to_gdfs(G, nodes=True, edges=False)# resave with the correct road node names
...
unconnected_test = list(nx.connected_components(G0))
if len(unconnected_test) > 1:
    ...
    while len(popped_multipoints) > 0:
        distances = []
        names = []
        dist_name_dict = {}
        for i in popped_multipoints:
            near_geom = nearest_points(list_multipoints[0], i)
            ...
            dist = near_geom[0].distance(near_geom[1])
            distances.append(dist)
            names.append((p1_name, p2_name, dist, line))
            dist_name_dict[dist] = (names[-1])
        min_dist = min(distances)
        min_name = dist_name_dict[min_dist]
        edge_connects.append(min_name)
...
#         dropped_nodes = set()
#         dropped_edges = set()
#         for i in nodes_out.index.values:
#             r_node = str(nodes_out['n_id'][i])
#             if r_node in combine_unconnected_lists:
#                 dropped_nodes.add(r_node)
#                 relevant_edges_u = edges_out[edges_out['uN'] == r_node]
#                 for u in list(relevant_edges_u.index):
#                     dropped_edges.add(u)
#                 relevant_edges_v = edges_out[edges_out['vN'] == r_node]
#                 for v in list(relevant_edges_v.index):
#                     dropped_edges.add(v)
#         nodes_out2 = nodes_out.drop(list(dropped_nodes))
#         edges_out2 = edges_out.drop(list(dropped_edges))
counter = 1
...
nodes.insert(5,'n_id',list(G.nodes))
...
def get_elevation_raster(bbox):
    ...
    dem.rio.to_raster(os.path.realpath(os.path.join(os.path.dirname('Donaldsville_Case'), '.')) + "\\city_dem.tif" )
    return os.path.realpath(os.path.join(os.path.dirname('Donaldsville_Case'), '.')) + "\\city_dem.tif"
    
dem_name = get_elevation_raster(city_bounds)
#save tif file for the R function and graphsetup

    
clusterfilename = 'Centralized_elevcluster_' + "Donaldsville" + '.txt'

# load the files for building locations 
clusterfile = os.path.realpath(os.path.join(os.path.dirname('Donaldsville_Case'), '.')) + '\\' + clusterfilename
...
counter = 1
for j in range(len(nodes_out)):
    node_x = float(nodes_out['lon'][j])
    node_y = float(nodes_out['lat'][j])
    node_xft = float(nodes_out['x'][j])
    node_yft = float(nodes_out['y'][j])
    node_elev = float(nodes_out['elevation'][j])
    node_cluster = int(nodes_out['cluster'][j])
    node_id = str(nodes_out['n_id'][j])
    node_demand = int(nodes_out['n_demand'][j])
