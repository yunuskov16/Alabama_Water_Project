...
#reads the cluster files from R, skipping the header and taking building  number, coordinates in lonlat, and elevation in meters
...
#note: the slopes in this function take the feadom node as the start point(i) and the to node as the endpoint (j)
...
wb = Workbook()
STEPPressurizeSystemSheet = wb.add_sheet("STEP_Pressurized_System")
STEPPressurizeSystemSheet.write(0, 0, 'Cluster_Name')
STEPPressurizeSystemSheet.write(0, 1, 'Obj1')
STEPPressurizeSystemSheet.write(0, 2, 'Obj2')
STEPPressurizeSystemSheet.write(0, 3, 'Obj3')
STEPPressurizeSystemSheet.write(0, 4, 'Obj4')
STEPPressurizeSystemSheet.write(0, 5, 'Obj')
STEPPressurizeSystemSheet.write(0, 6, 'Objective + Additional Costs')
...
ngroups = 5 # number of clusters fit by the clustering algorithm
for cluster in range(1, (ngroups-1)):
    ...
    STEPPressurizeSystemSheet.write(cluster, 1, obj1.getValue())
    STEPPressurizeSystemSheet.write(cluster, 2, obj2.getValue())
    STEPPressurizeSystemSheet.write(cluster, 3, obj3.getValue())
    STEPPressurizeSystemSheet.write(cluster, 4, obj4.getValue())
    STEPPressurizeSystemSheet.write(cluster, 5, m.objVal)
    ...
    # modelname = "Uniontown_" + tot_clust_str + clustername + "STEPPressurizedSystem" + ".txt"
    # #m.write(modelname)
...
wb.save('v2_MINLP_STEP_Pres_updated.xls')